...
public class GoogleAnalyticsMPV3
{
	...
	readonly Dictionary<string,object> trackerValues	= new Dictionary<string,object>();
	...
	public void SetTrackerVal(string field,object value)
	{
	...
	private string AddTrackerVals()
	{
		...
		string vals = "";
		foreach(var pair in trackerValues)
		{
	...
	*/
	public IEnumerator HandleWWW(WWW request)
	{
		while(!request.isDone)
		{
			...
			string status;
			if(request.responseHeaders.TryGetValue("STATUS",out status))
			{
	...
	private string AddRequiredMPParameter(string parameter,object value)
	{
		if(!trackingCodeSet)
			return "";
	...
	private string AddRequiredMPParameter(string parameter,string value)
	{
		if(!trackingCodeSet)
			return "";
	...
	private string AddOptionalMPParameter(string parameter,object value)
	{
	...
	private string AddOptionalMPParameter(string parameter,string value)
	{
	...
	private string AddCustomVariables(HitBuilder builder)
	{
		if(!trackingCodeSet)
		{
			return "";
		...
		string url	= "";
		foreach(var entry in builder.CustomDimensions)
		{
			if(entry.Value != null)
				url += Fields.CUSTOM_DIMENSION.ToString() + entry.Key + "=" + WWW.EscapeURL(entry.Value.ToString());
		}

		foreach(var entry in builder.CustomMetrics)
		{
			if(entry.Value != null)
				url += Fields.CUSTOM_METRIC.ToString() + entry.Key + "=" + WWW.EscapeURL(entry.Value.ToString());
		}
